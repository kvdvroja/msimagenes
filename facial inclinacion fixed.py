...
class Facial:
    ...
    def Validar(self):
        try:
            ...
            if self.validar_cabeza_y_hombros_rectos(image):
                respuesta_cabeza_recta = True
            else:
                msg += "<tr><td><i class='fa fa-circle' aria-hidden='true' style='color: #ff2d41;font-size: 20px;'></i></td><td class='ytradre_tbl_td'> La imagen muestra&nbsp;<b style='color:red;'>inclinación</b> de cabeza o cuerpo</td></tr>"
                respuesta = False
    ...
    def validar_cabeza_y_hombros_rectos(self, image):
        ...
        hombros_rectos = self.validar_inclinacion_hombros(image)
    ...
    def validar_inclinacion_hombros(self, image):
        ...
        with self.mp_pose.Pose(static_image_mode=True) as pose:
            ...
            # Asegurar que los x estén dentro del ancho
            x_izq = np.clip(x_izq, 0, ancho - 1)
            x_der = np.clip(x_der, 0, ancho - 1)

            def buscar_pixel_hombro_mas_alto(x):
                for y in range(int(alto * 0.5), alto):  # Solo parte inferior
                    pixel = image[y, x]
                    if not all(245 <= c <= 255 for c in pixel):  # No es fondo blanco
                        return y
                return alto  # Si no encuentra, devuelve abajo del todo

            y_izq = buscar_pixel_hombro_mas_alto(x_izq)
            y_der = buscar_pixel_hombro_mas_alto(x_der)

            diferencia_y = abs(y_izq - y_der)
            umbral_diferencia = int(alto * 0.04)  # por ejemplo: 11px en 288
